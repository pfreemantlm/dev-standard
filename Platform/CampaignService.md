...
campaign ID, and the placements should be supplied as multiple "placements[]" arguments. Each item in the "placements[]" arguments refers to a single placement; supplying a package ID (for a parent used in packageItems) will not automatically create tags for everything underneath it. The "approval" value should be set to indicate whether new tags should be created already approved, a value of 0 will mean new tags once created will require approval before being released to vendors; if "approval" is not set, or given a value of 1, new tags will automatically be approved and available to vendors upon creation.
