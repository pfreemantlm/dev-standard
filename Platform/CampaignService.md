...
and an already-existing "campaignID" of the appropriate format. The "nolivesync" value can be set to 0 indicate that after a successful upload, the data should not be automatically sent to the live campaign data.
...
campaign ID, and the placements should be supplied as multiple "placements[]" arguments. Each item in the "placements[]" arguments refers to a single placement; supplying a package ID (for a parent used in packageItems) will not automatically create tags for everything underneath it. The "approval" value should be set to indicate whether new tags should be created already approved (0 is to not approve, 1 is to create tag as approved).

Sample input:

    campaignID=642001&placements[]=3300081&placements[]=3300082&approval=0
